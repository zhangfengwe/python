...
import requests
from python.study.other.util import decorator
...
headers = [
    ...
    "Mozilla/5.0 (iPod; U; CPU iPhone OS 4_3_3 like Mac OS X; en-us) AppleWebKit/533.17.9 "
    "(KHTML, like Gecko) Version/5.0.2 Mobile/8J2 Safari/6533.18.5",
    ...
    "Mozilla/5.0 (hp-tablet; Linux; hpwOS/3.0.0; U; en-US) AppleWebKit/534.6 "
...
proxies = {
    ...
    'https': 'https://113.124.93.45:9999',
    'https': 'https://106.110.212.112:9999'
}
...
def get_response(url, timeout):
    try:
        response = requests.get(url, headers=kv_local, proxies=proxies)
        if response.status_code != requests.codes.ok:
            logger.error('{} get response is fail, retcode is {}'.format(url, response.status_code))
            # return None
